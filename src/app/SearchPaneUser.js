define([
    ...
    "dojo/text!./templates/SearchPaneUser.html",
    "dojo/domReady!"
], function(parser, declare, window, lang, dom1, on, json, request,domStyle,_WidgetBase, _OnDijitClickMixin, _TemplatedMixin, _AttachMixin, _WidgetsInTemplateMixin, TabContainer, ContentPane, LayoutContainer, StackContainer, StackController, Select, Button, ValidationTextBox, TextBox, Textarea, SimpleTextarea, Dialog, Grid, OnDemandGrid, Selection, CellSelection, Keyboard, TooltipDialog, Tooltip, popup, fx, BorderContainer,  CustomizedTooltipDialog, RecreateRecordsetDialog, template, dom) {
     console.log("ENTERED SearchPaneUser");
     ...
     return declare([_WidgetBase, _OnDijitClickMixin, _TemplatedMixin, _WidgetsInTemplateMixin], {
        ...
        showRecordsList: function(resp)
        {
          ...
          var recordsGrid = new CellSelectionGrid({columns: columns, selectionMode: "single"}, "my_records_user");
        ...
        recordSelection: function(evt)
        {
          ...
          if(evtType == "dgrid-select")
          {
            ...
            if(column == "gpid")
            {
              // Show RID in its pane
              var gpidPane = window.global.app.widgets.GpidPaneUser;
              var field = gpidPane.gpid_input;
              field.set('value', gpid);
        ...
        buildRecordsetsList: function(resp)
        {
          ...
          var recordsetsGrid = new CellSelectionGrid({columns: columns, selectionMode: "single"}, "my_recordsets_user");
        ...
        { 
          console.log("recordsetSelection", evt);
          var evtType = evt.type;
          if(evtType == "dgrid-select")
          {
            var cells = evt.cells[0];
            ...
            if(column == "name")
            {
              if(this.pickType == 0) // Picking for displaying
              {
                ...
                dojo.byId("download_link_user").style.visibility = 'visible';
              }
        ...
        attachTooltipDialogs: function()
        {
          ...
          for(var i=0; ; i++)
          {
            var node=dom1.byId("my_recordsets_user-row-" + i);
            if(node == null) break;
        ...
        {     
          ...
          if(this.currentSearchType == "special")
          {
            ...
            for (var property in this)
            {
              ...
              {
                if(property.substr(0,8) == "user_dys")
                {
        ...
        showSaveRecordsetPane: function(resp)
        {
          ...
          var node = dojo.byId("gpids_count_user");
        ...
        combineRecordsetsFocused: function()
        {
          ...
          if(this.firstCall == 0)
          {
            ...
            this.pickedRecordsetsGrid = new CellSelectionGrid({columns: columns1, selectionMode: "single"}, "combine_recordsets_user");
            this.pickedRecordsetsGrid.refresh(this.picked_data);
            this.pickedRecordsetsGrid.renderArray(this.picked_data);
          }
          this.finishPicking.set('disabled', true);
        ...
        postCreate: function()
        {
          ...
          for (var property in this)
          {
            if(this.hasOwnProperty(property))
            {
              if(property.substr(0,8) == "user_dys") this[property].validator = lang.hitch(this, this.strValidator);
            }
          }
          ...
          window.global.app.widgets.SearchPaneUser = this;
          this.findButton.set('disabled', true);
        }
    });
});
